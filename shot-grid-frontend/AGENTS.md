...
- 镜头表在“制作内容”列后、资产树表在“说明”列后展示“开始时间 / 结束时间”，表头省略“预期”但仍表示管理员设置的预期时间，不要求管理权限；两表的“时间状态”列使用 Element Plus `fixed="right"` 固定在“制作人”列前。时间状态复用 `taskTimeReminder` 和 `useCurrentTime`，仅基于本行实际预期结束时间，独立于制作状态。无预期时间显示“— / 未设置时间”，已完成停止预警；父资产没有整资产任务时间，三列显示“—”，展开后分别显示各活动分项的时间，禁止用某一分项代替父资产时间或逐资产预取。镜头列表直接返回顶层 `expectedStartTime/expectedEndTime`，不为每行另查详情。
