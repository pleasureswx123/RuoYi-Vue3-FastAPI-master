...
- 资产或制作分项可执行动作必须读取后端 `allowedActions`，再与当前平台权限取交集；页面不得根据角色名、状态文案或是否显示按钮自行合成写权限。资产动作代码为 `asset.edit`、`assetItem.add`、`asset.archive`，制作分项动作代码为 `assetItem.edit`、`assetItem.archive`、`assetItem.delete`、`task.assign`。
...
- 镜头、资产及制作分项的手工创建和 Excel 导入只能创建生产对象与关系，默认聚合状态为 `unassigned`，不得创建对象目录 Outbox、接收制作人或顺带创建任务；资产创建时只冻结稳定目录身份。第一次委派创建非空 `assignee_user_id`、状态为 `not_started` 的唯一任务；镜头和资产任务开始后都先进入 `preparing`，目录 Outbox 成功后再进入 `in_progress`。同一父资产的多个制作分项共享一个资产目录，后端开始任务时锁定父资产并复用同一幂等目录操作。不可变版本正式提交后进入 `pending_review`，审核通过后版本为 `final` 且任务为 `completed`，退回后任务为 `revision`。
...
- 当前 Shot Grid head 为 `20260826_22`；20 增加版本轮次内多候选文件、候选级媒体派生、审核候选选择审计，并把既有版本回填为候选 01；21 增加审核通过后的最终交付 Outbox 与 `FINAL/FINAL.json` 投影；22 自动选择并回填单候选版本。历史和候选 NAS 文件均不改名。该增量链不把全平台 Alembic 链变成可从真正空库独立建库的 baseline。
