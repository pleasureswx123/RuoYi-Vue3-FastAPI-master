...
- 正式设计契约见 `docs/superpowers/specs/2026-08-31-shot-grid-scheduling-timeline-gantt-design.md`。当前实现通过 `ScheduleBoard` 接入项目、镜头和资产入口，并以 GPL-3.0 `@svar-ui/vue-gantt` 的项目适配层隔离甘特渲染；依赖、迁移、API 和页面存在不等于生产数据迁移、真实负载性能或完整 E2E 已通过。
