...
- PostgreSQL `20260828_24` 新增两列可空 `TIMESTAMP(0) WITHOUT TIME ZONE`，既有任务不伪造时间。开工携带时间后由服务端同事务更新优先级/时间/任务状态/审计，`dueDate` 只保留为预期结束日期的兼容筛选投影。
...
- 正式设计契约见 `docs/superpowers/specs/2026-08-31-shot-grid-scheduling-timeline-gantt-design.md`。该文档获批不等于依赖、迁移、API、页面、性能或 E2E 已实现；实施前必须先通过开源渲染核心适配门禁。
...
- 当前 Shot Grid head 为 `20260828_24`；20 增加版本轮次内多候选文件、候选级媒体派生、审核候选选择审计，并把既有版本回填为候选 01；21 增加审核通过后的最终交付 Outbox 与 `FINAL/FINAL.json` 投影；22 自动选择并回填单候选版本；23 仅更名标准开工权限菜单，管理角色授权仍由平台管理员显式核对配置；24 增加任务预期时间范围，不回填历史时间或自动完成任务。历史和候选 NAS 文件均不改名。该增量链不把全平台 Alembic 链变成可从真正空库独立建库的 baseline。
