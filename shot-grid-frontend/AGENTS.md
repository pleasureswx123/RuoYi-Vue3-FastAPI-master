...
- 镜头主产出物业务文件名固定为 `{项目缩写}_EP{集号至少3位}_{场次号至少3位}_S{镜头号至少3位}_{制作人用户昵称}_V{版本号至少3位}_{服务端毫秒时间戳}.{mp4|mov}`，例如 `WGZR_EP001_001_S001_YJF_V001_1786094626499.mp4`。文件名版本号必须与正式版本记录一致。
- 资产主产出物业务文件名固定为 `{项目缩写}_Asset_{Character|Environment|Prop}_{资产名称}_{制作分项}_{制作人用户昵称}_V{版本号至少3位}_{服务端毫秒时间戳}.{jpg|png}`，例如 `WGZR_Asset_Environment_动力舱室内_动力舱恐怖气氛主视角_YJF_V001_1786094626499.jpg`。资产名称和制作分项都必须安全规范化，且制作分项取任务所属 `sg_asset_item` 的稳定值。
...
- 当前 Shot Grid head 为 `20260825_19`；15 将镜头目录冻结延迟到任务开始并增加 `preparing`，16 校验场内连续编号，17 增加审核问题私有草稿，18 增加项目永久删除队列，19 将资产目录创建延迟到资产任务开始并允许资产任务进入 `preparing`。该增量链不把全平台 Alembic 链变成可从真正空库独立建库的 baseline。
...
- 版本审核一级页和详情已接真实 `auto_single` 与 `manual_batch` 审核 API，并实现“来源版本问题 → 制作人逐条处理说明 → 新版本逐条确认 → 新问题”的闭环，同时保留三种审核决定、鉴权媒体、时间点、四类归一化批注和 A/B 对比。功能代码完成不替代用户手动旅程，也不能替代真实 MP4/MOV 的 206/416 浏览器验收。
