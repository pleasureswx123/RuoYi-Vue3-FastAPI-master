...
- 镜头与资产任务分配窗口聚焦制作人和制作信息，不再采集优先级与日期；开工统一复用 `TaskStartDialog`，展示当前制作人及完整制作信息，再确认优先级、预期制作时间和线下开工条件。
- 镜头与资产开工窗口不展示预期时间用途的长说明，线下开工条件默认勾选；用户取消勾选后仍须由表单校验阻止开工，只有点击“确认开工”才发送请求。时间提醒和后端人工确认规则保持不变。
- 预期制作时间必须使用 `ElDatePicker type="datetimerange"`，字段为 `expectedStartTime/expectedEndTime`，采用业务本地时间、秒级精度；过去日期/时间禁选，提交时重新校验开始不早于当前时间、结束严格晚于开始。使用 `ElForm.validate()`，不使用原生 submit；忙碌、失败、重复点击、关闭和跨项目/查询迟到结果仍须受控。
