...
- 2026-08-28 起，镜头号统一为至少四位纯数字：第 1 镜 `0001`、第 12 镜 `0012`，超过四位不截断。数据库 `shot_no`、`sequencePosition` 和稳定 `shot_id` 不变；新建目录冻结为 `{sceneNo:03d}_{shotNo:04d}`，新业务文件名使用四位数字镜头段。Excel 当前模板 `shot-v3` 的编号单元格使用文本格式，导入兼容正整数数值/纯数字文本并补齐显示，不再接受 S 前缀。此次不回填旧记录、不重命名已有目录/文件、不执行旧数据迁移；已冻结的路径仍按原快照读取。
- 当前镜头 Excel 下载模板为后端打包的匿名 `module_shot_grid/resources/templates/shot-v3.xlsx`：每个可见 Sheet 表示一集，Sheet 名为 `EP001` 等格式，主数据区为从 A1 开始的 A:O 15 列且不含制作人；冻结 SHA-256 为 `23FF46F60BD4E52A7C3B9350F89882BB18963C92823AC40AFE601AC1553204F8`。旧 `shot-v1.xlsx`、`shot-v2.xlsx` 只作为历史资源保留，不再由服务下载。模板不得重新引入驱动器路径、`file:` URI、UNC、个人/组织或应用元数据。
...
- “序”固定规范化为 `scene_no=0` 和场次代码 `000`；镜头号在场内唯一，不同场次可分别从 `0001` 开始；焦段按文本保存以兼容 `35/25` 等组合值。
...
- 2026-08-21 后，`shot_id` 是稳定内部主键且与 NAS 无关；`sequencePosition`、`shotNo` 和 `数字镜头号` 是同一场内镜序的不同投影，第 N 镜必须为 `{N:04d}`。新建/导入镜头不立即建 NAS 目录，任务开始时才冻结 `{sceneNo:03d}_{shotNo:04d}` 并从 `preparing` 等待 Outbox 成功后进入 `in_progress`。拖拽前必须加载完整场次且仅在被移动区间全部镜头未开始制作时允许；`not_started` 可排，已开始状态或已有版本/文件整体禁止。拖拽成功后自动同步 `数字镜头号`，不再暴露独立的常规“重新编号”动作。删除后端从最早删除位置到场尾重新检查同一门禁并连续化剩余镜头；受影响区间已有冻结目录时拒绝删除，前端必须展示后端错误，不能假装只释放编号。
...
- 子行展示自身的名称、说明、最新版本缩略图、制作人、状态和最新版本号；缺图不借用父资产或其他分项图片。操作列按 `allowedActions` 与平台权限交集直接显示开始任务、分配/改派任务、编辑分项和删除分项，复用详情的原有表单、确认流程及共用开工逻辑；点击后先重读详情，冻结目标、锁版本和操作代次，项目或查询切换后取消旧请求并隔离迟到事件。分项详情仍打开资产抽屉并定位目标；已归档分项从详情查看，不计入树表活动子行或父级分页。
- 制作分项名称 `productionItem` 为空或全空白且允许编辑时，子行显示“补齐制作分项”替代“编辑分项”，打开原编辑表单；保存成功刷新父资产与已展开分支。完整性判断与后端稳定名称规则一致，缺少可选说明、备注、缩略图或版本不视为未补齐。
...
- 资产树表父行展示“资产描述”（`sg_asset.description`），子行隐藏资产描述，只展示自身“分项补充要求”（`sg_asset_item.description`），没有补充要求显示“—”。父描述为空显示“暂无资产描述”，不根据子项内容推断或回填。导入预览与详情分项卡片使用同一组合组件，仍同时展示资产描述和分项补充要求，完全相同的可见内容只在展示时去重；编辑分项时只读展示所属资产描述，输入框只保存该分项的补充要求。隐藏仅作用于列表展示，不改写数据或任务、分配、开工及审核中的完整制作信息。说明使用 `ElText.line-clamp` 默认最多三行，实际溢出后用 `ElButton` 展开/收起；分项数量放在父名称旁，版本放在子项缩略图下，父状态只显示汇总。父资产全部可用操作直接使用 `ElButton` 展示，不再收进“更多”；编辑、删除仍保留原权限、加载、上下文门禁及确认流程，操作列允许换行。
- 镜头、资产和制作分项的表格操作按钮统一复用 `TableActionButton`，由原生 `ElButton size="small" round` 提供尺寸、加载和禁用行为，始终显示“图标＋中文文字”及中文 `aria-label`。开工为主色实底；分配/改派为蓝色朴素按钮；详情/编辑为中性朴素按钮；补齐分项为警告色 `plain dashed`；删除为危险色朴素按钮。自定义 `color` 复用主题变量，`dark` 绑定现有 Pinia 主题 Store；通过组件颜色变量保证普通、悬停、按下、键盘焦点和禁用态可辨识，不手写按钮交互。资产操作列宽度为 480px，镜头操作列宽度为 420px，保留换行和表格横向滚动。操作按钮不使用 `ElTooltip` 或 `title` 显示悬停提示；必要的禁用原因保留为 `aria-description`，不能因展示调整而改变权限或点击处理链。
- 两张表的制作人使用中性主文字，无负责人时弱化。父资产汇总及分项、镜头状态统一复用现有七色主题变量和 `ElTag size="small" effect="light" round`：待分配琥珀、待开工灰、目录准备青、制作中蓝、待审核紫、修改中红、已完成绿；目录失败额外显示红色异常标签。明亮模式加深标签文字，暗黑模式使用深底亮字，不以 `effect="dark"` 代替页面主题适配。资产状态列宽度为 200px，优先横向排列，空间不足时换行，不隐藏计数。
- 资产表至少包含类型、名称、制作分项、描述、备注、状态、制作人；任务描述、状态和制作人从当前制作分项唯一任务聚合。
- 资产导入的“描述/资产描述”属于父资产，预览必须展示 `normalized.assetDescription`；“分项补充要求/制作分项描述”为可选独立列，展示 `normalized.itemDescription`，不能二选一回退而隐藏另一层。没有补充列时分项补充为空，备注仍属于对应分项。后续导入按新映射执行，不迁移已有项目；页面统一使用“资产描述”“分项补充要求”，任务、开工与审核视图保持一致。
...
- 资产列表中的批量分配以资产为选择单位、以其全部活动制作分项任务为实际写入单位；父资产 `allowedActions` 仅在全部活动分项均可分配时聚合返回 `task.assign`。弹窗确认前应读取详情取得每个 `assetItemId/taskLockVersion`，后端按制作分项执行单事务批量创建或改派。资产删除仅允许无镜头引用、无版本、无非 `committed` 提交且全部未删除分项的任务均未开始；已归档分项同样参与阻断判断。任一分项进入 `preparing` 或后续状态，均禁止删除该分项及父资产；删除事务仅软删除未开始任务、活动分项和父资产，保留已有归档历史。
- 镜头列表操作列提供“分配任务”或“改派任务”，点击后重读目标镜头详情并复核 `task.assign` 与平台权限，复用 `ShotAssignDialog`；使用冻结的当前任务锁号，成功后刷新列表，项目或查询切换必须关闭旧弹窗并忽略迟到结果。
...
- 镜头和制作分项统一仅未开工可普通改派：未分配显示“分配任务”，`not_started` 显示“改派任务”；目录准备中及后续状态均隐藏普通改派，管理人员也不能绕过。列表、详情及批量操作以 `allowedActions` 与平台权限交集为准，缺少动作投影不得放行；父资产仅在全部活动分项均可分配时支持批量分配。页面打开后状态变化由后端锁内重新校验并返回冲突，批量失败整体回滚，不自动跳过已开工目标。
...
- 资产类型看板使用 Element Plus 卡片头部插槽，资产入口保留 `ElButton` 点击和键盘行为；缩略图与文字分栏，名称最多两行，分项数量与状态统计分层显示。存在 `itemStatusCounts` 时只展示带数量的分项状态标签，不重复聚合状态；没有计数时保留聚合状态兜底。标签允许换行，开工入口不挤占名称宽度；按钮普通、悬停和按下颜色通过主题变量交给 Element Plus，兼容明暗模式。

- 资产详情头部使用 `ElCard` 头部插槽承载资产身份和操作按钮，操作区随容器宽度换行，不能挤占正文宽度。资产描述独占正文通栏；分项预览用 `ElCard` 自适应网格显示，随可用宽度换行，名称用 `ElText.line-clamp=2`，不依赖悬停提示。保留活动分项顺序、独立缩略图、版本状态、权限与原操作流程；已有分项状态数量时不重复聚合状态标签。

### 4.2.1 预期制作时间与提醒

- 镜头与资产任务分配窗口聚焦制作人和制作信息，不再采集优先级与日期；任务建立后可由统一排期视图提前设置当前计划。开工统一复用 `TaskStartDialog`，展示当前制作人及完整制作信息，再确认优先级、时间和线下开工条件：已有计划时只读沿用，尚无计划时才建立首次完整范围。
- 镜头与资产开工窗口不展示预期时间用途的长说明，线下开工条件默认勾选；用户取消勾选后仍须由表单校验阻止开工，只有点击“确认开工”才发送请求。时间提醒和后端人工确认规则保持不变。
- 新建首次预期制作时间必须使用 `ElDatePicker type="datetimerange"`，字段为 `expectedStartTime/expectedEndTime`，采用业务本地时间、秒级精度；从开工窗口新建时过去日期/时间禁选，提交时重新校验开始不早于当前时间、结束严格晚于开始。已有计划在开工窗口只读，不因开始已过去阻止开工；排期编辑可保留过去开始以真实表达延期。表单使用 `ElForm.validate()`，不使用原生 submit；忙碌、失败、重复点击、关闭和跨项目/查询迟到结果仍须受控。
- 这两个时间只表达管理员期望，不是实际开工/完成时间。确认开工依然按原规则立即放行并准备目录；不得到达预期开始时自动开工，也不得到期自动完成、暂停、改派或禁止提交。早于预期开始或晚于预期结束均不额外限制已经可提交的任务。
- 制作人的工作台及任务详情显示独立“时间提醒”，不与 `taskStatus` 混用：剩余超过 24 小时为正常，0 至 24 小时（含 24 小时）为临近结束，到达结束时间后为已延期；已完成任务停止催办，无时间显示未设置。页面每 30 秒及恢复可见时本地重算，不进行额外网络轮询、不修改数据库；历史 `dueDate` 按当天结束兼容展示。
- 镜头详情的任务卡片及资产详情的各制作分项同样复用 `TaskTimeReminder`，显示各自预期开始至结束时间和提醒；仅有旧 `dueDate` 时注明“原截止日期”，不伪造开始时间，未分配分项不展示任务时间。
- 镜头表在“制作内容”列后、资产树表在“说明”列后展示“开始时间 / 结束时间”，表头省略“预期”但仍表示管理员设置的预期时间，不要求管理权限；两表的“时间状态”列使用 Element Plus `fixed="right"` 固定在“制作人”列前。时间状态复用 `taskTimeReminder` 和 `useCurrentTime`，仅基于本行实际预期结束时间，独立于制作状态。无预期时间显示“— / 未设置时间”，已完成停止预警。父资产开始、结束两列仍显示“—”，时间状态通过列表的 `itemTimeGroups` 汇总全部活动分项，同类合并计数，按已延期、临近结束、正常、未设置时间、已完成排序，无分项显示“—”；使用小号 Element Plus 标签，尽量同排，空间不足换行。收起时也必须完整显示，不依赖懒加载分支或逐资产预取；每 30 秒及恢复可见时本地重算，不增加网络轮询。镜头列表直接返回顶层 `expectedStartTime/expectedEndTime`，不为每行另查详情。
- PostgreSQL `20260828_24` 新增两列可空 `TIMESTAMP(0) WITHOUT TIME ZONE`，当前 head `20260831_25` 进一步增加排期基线和历史；既有无计划任务不伪造时间。开工首次携带时间时由服务端同事务更新优先级、当前时间、基线、任务状态和审计，`dueDate` 只保留为预期结束日期的兼容筛选投影。

### 4.2.2 项目排期、泳道时间线与甘特图

- 2026-08-31 起，Shot Grid 排期采用“`sg_task.expected_start_time/expected_end_time` 当前计划 + 首次完整排期冻结基线 + 只追加结构化改期历史”的统一领域模型。镜头、资产制作分项和项目综合排期必须共用同一组读取/写入 API 与 `ScheduleBoard`，不得为页面或第三方甘特组件复制第二套任务、时间、权限或审计事实。
- 人员泳道用于查看当前项目同一负责人任务分布与撞期，任务甘特用于查看任务层级、当前计划和基线偏差；两者共用筛选、自然时间轴、日/周/月缩放、未排期池和显式编辑模式。周末与节假日均可排，不引入工时、容量、任务依赖、关键路径、自动调度或虚假完成百分比。
- 排期修改只允许具备 `shotgrid:task:schedule` 且属于项目 `director` 或 `has_all_scope` 的管理人员；制作人员只读。拖入其他人员泳道不得改派，必须回弹并引导使用既有委派/改派流程。已完成任务、归档/完成项目及失效目标只读，改期不得改变任务状态、开工、目录、版本或审核链。
- 同项目同负责人未完成活动任务的当前计划按半开区间 `[start,end)` 判断重叠。冲突只警告，前端必须展示后端最新冲突集合并二次确认；服务端在项目协调锁和任务乐观锁内复算，集合变化或锁版本变化时拒绝旧确认。当前计划、`dueDate`、任务版本、结构化历史和平台操作日志必须同事务写入，失败整体回滚。
- 页面默认只读，管理人员显式进入编辑模式后才允许水平拖动和调整边界；松开只产生草稿并打开 Element Plus 表单填写原因，不能直接落库。第三方开源甘特核心只负责时间网格、拖拽和虚拟渲染，必须通过项目适配层隔离；筛选、表单、弹窗、抽屉、空态和提示继续使用 Element Plus，禁止依赖商业版能力或绕开 Axios/Pinia/Vue Router。
- 排期窗口以业务本地自然日期连续计算并偏重未来规划：日视图显示过去 7 天、今天和未来 23 天；周视图按周一对齐，显示过去 4 周、当前周和未来 8 周；月视图按月初对齐，显示过去 3 个月、当前月和未来 9 个月。工具栏使用 `ElDatePicker type="daterange"` 展示包含首尾的日期，查询内部仍使用次日零点作为排他结束边界；“回到今天”按当前缩放恢复对应窗口，左右导航移动一个完整窗口。显式 URL 范围和用户手动范围在未触发缩放或“回到今天”前不得被默认值覆盖。
- 正式设计契约见 `docs/superpowers/specs/2026-08-31-shot-grid-scheduling-timeline-gantt-design.md`。当前实现通过 `ScheduleBoard` 接入项目、镜头和资产入口，并以 MIT `@svar-ui/vue-gantt` 的项目适配层隔离甘特渲染；依赖、迁移、API 和页面存在不等于生产数据迁移、真实负载性能或完整 E2E 已通过。
...
- 镜头候选主产出物业务文件名固定为 `{项目缩写}_EP{集号至少3位}_{场次号至少3位}_{镜头号至少4位}_{制作人用户昵称}_V{版本号至少3位}_{候选号至少2位}.{mp4|mov}`。文件名版本号和候选号必须与正式版本候选一致。
- 资产候选主产出物业务文件名固定为 `{项目缩写}_Asset_{Character|Environment|Prop}_{资产名称}_{制作分项}_{制作人用户昵称}_V{版本号至少3位}_{候选号至少2位}.{jpg|png}`。资产名称和制作分项都必须安全规范化，且制作分项取任务所属 `sg_asset_item` 的稳定值。
- 镜头和资产新成果文件名以版本号、候选号及扩展名结尾，不再追加时间戳。批次生成时间仍保存为内部元数据；同一幂等请求复用已生成值和业务文件名，已有提交（包括失败重试）及 NAS 文件不改名，禁止覆盖规则保持不变。
...
- 当前 Shot Grid head 为 `20260831_25`；20 至 22 完成多候选、最终交付和单候选规范化；23 仅更名标准开工权限菜单；24 增加任务预期时间范围；25 增加不可变首版排期基线、只追加改期历史和独立排期权限。历史和候选 NAS 文件均不改名。该增量链不把全平台 Alembic 链变成可从真正空库独立建库的 baseline。
...
- 服务异常页的“重新加载”必须读取并净化 `retry` 站内地址，经 Vue Router 返回原业务路由，让既有守卫重新校验账号和导航；不得仅刷新公开的异常页本身。缺失、站外或指向当前异常页的目标回退 `/`，保留原项目等查询参数；重试期间显示 loading、防止重复点击，不清除登录，继续遵守 401/403 门禁。
...
- 镜头列表以 `GET /shot-grid/projects/{projectId}/shots` 为唯一数据源，项目、集、场次、状态和制作人筛选与服务端分页在表格、卡片、故事板之间保持一致；`sortOrder` 默认排序必须先按集、再按场次、最后按场内镜头，不得跨集或跨场交错。镜头页提供独立“新建集”和“新建场次”动作，场次允许 `000/序`；新建镜头只选择场内插入位置，服务端派生 `数字镜头号`。表格首列允许选择当前页中可分配或可删除的镜头；只有已筛选到具体集和具体场次、没有关键字/状态/制作人附加筛选、使用升序表格视图且前端已加载完整场次时，才允许通过 `SortableJS` 手柄拖拽。完整场次最多自动加载 2000 条，加载期间或超过上限时禁用拖拽并给出原因；完整加载后隐藏分页，提交的位置是整场位置而不是页内下标。拖拽调用 `PUT /shot-grid/projects/{projectId}/shots/{shotId}/sequence` 提交 `sequencePosition/lockVersion`，服务端锁定目标场次并同步 `sequencePosition/shotNo/数字镜头号`；普通编辑不得跨场移动或直接改号，且只允许未分配或任务仍为 `not_started` 的镜头，进入 `preparing/in_progress/pending_review/revision/completed` 后列表与详情都不展示编辑入口，后端普通更新接口也必须在锁内拒绝。拖拽成功或失败都重新读取列表，且不再向用户暴露独立常规重编号动作。批量分配使用 `POST /shot-grid/projects/{projectId}/shots/batch-assign`；列表工具栏只保留批量分配/重新分配按钮，弹窗统一提交 `assigneeUserId` 与每行 `shotId/taskLockVersion`。行尾提供详情、开始任务、编辑、删除；批量删除使用 `POST /shot-grid/projects/{projectId}/shots/batch-delete` 并提交每行 `shotId/lockVersion`。单条和批量删除都必须由服务端锁定整场，从最早删除位置到场尾重查未开始制作/版本/文件门禁；通过后在同一事务内软删除 `not_started` 任务和目标镜头，并把剩余活动镜头连续化为 `0001..NNNN`。受影响区间已有冻结目录时以 `SG_SHOT_DELETE_DIRECTORY_EXISTS` 整体拒绝，禁止隐式改名。项目切换、筛选变化和卸载会取消旧请求，防止跨项目迟到响应覆盖当前状态；同时关闭创建/导入/编辑弹窗并清空旧项目预检 Token、幂等键、选中行和问题明细。创建、导入、编辑和分配弹窗携带单调递增的 `operationGeneration`，旧请求迟到不得关闭新弹窗或刷新当前上下文。
...
- 任一未删除分项任务开工（含目录准备中、已完成、已归档分项）后，父资产描述只读，排序和内部备注仍可编辑。编辑弹窗以详情 `descriptionLocked` 为准，缺失该字段时暂不允许改说明；不得仅凭活动分项聚合状态推断。使用 `ElInput readonly` 和一条原因提示，保留 `asset.edit` 与平台权限交集。普通 PUT 仍提交完整快照，锁定时带回打开弹窗时的原说明；409 `SG_ASSET_DESCRIPTION_LOCKED` 恢复原说明并锁定字段，保留排序/备注草稿，沿用原锁号重试，不能偷换新锁。保存从 `ElForm.validate()` 前起防重复提交，卸载后丢弃迟到结果；服务端项目锁内校验是最终门禁。
...
### 镜头号区间筛选的场次前置条件（2026-09-08）

镜头管理页面仅在选中具体场次后启用镜头号区间输入；未选场次时禁用输入且查询不携带区间参数。切换或清空场次时同步清除起止镜头号、已应用区间及字段校验提示，避免不同场次的相同镜头号产生筛选歧义。
...
编辑镜头使用右侧 Element Plus Drawer，复用原有 ElForm 校验、保存、错误提示和关闭重置逻辑；保存期间禁止关闭，窄屏抽屉占满宽度。新建镜头继续使用弹窗。


镜头编辑未保存保护：关闭按钮、遮罩、Esc 和取消按钮统一检查可编辑字段；存在未保存修改时通过 Element Plus MessageBox 确认放弃，取消确认保留全部输入。无修改、改回原值和保存成功后不提醒；保存中禁止关闭，同一时刻只允许一个关闭确认。
