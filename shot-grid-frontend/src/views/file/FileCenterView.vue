    ...
    <template v-else>
      ...
      <el-card v-if="selectedProjectId" class="file-panel" shadow="never">
        ...
        <el-form ref="fileFilterForm" :model="filters" class="file-filters" size="large" aria-label="文件筛选">
          <el-form-item prop="fileRole"><el-select v-model="filters.fileRole" class="sg-select" placeholder="全部用途" aria-label="按文件用途筛选" @change="submitFilters"><el-option label="全部用途" value="" /><el-option label="审核文件" value="review_media" /><el-option label="原始生成文件" value="source_original" /><el-option label="修复后文件" value="source_repaired" /><el-option label="首帧" value="first_frame" /><el-option label="尾帧" value="last_frame" /><el-option label="参考文件" value="reference" /></el-select></el-form-item>
