...
<div class="filter-bar"><el-input v-model="store.query.keyword" clearable placeholder="搜索镜头号或制作内容" @input="search" /><el-input v-model="store.query.episodeId" placeholder="集 ID" /><el-input v-model="store.query.sceneId" placeholder="场次 ID" /><el-input v-model="store.query.assigneeUserId" placeholder="负责人 ID" /><el-select v-model="store.query.status" clearable placeholder="聚合状态"><el-option v-for="option in productionStatusOptions" :key="option.value" :label="option.label" :value="option.value"/></el-select><el-select v-model="store.query.orderBy"><el-option label="制作顺序" value="sortOrder"/><el-option label="最近更新" value="updateTime"/></el-select></div>
