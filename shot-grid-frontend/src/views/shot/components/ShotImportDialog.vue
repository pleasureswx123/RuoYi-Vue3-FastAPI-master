...
import { groupPreviewRows, selectablePreviewRows, shotErrorState } from '@/views/shot/shotPresentation'
...
const validRows = computed(() => selectablePreviewRows(preview.value?.rows || []))
...
async function runPreview() {
  ...
  try {
    ...
    selectedKeys.value = new Set(selectablePreviewRows(response.data?.rows).map(row => rowKey(row)))
  } catch (error) {
...
function toggleSheet(rows) {
  ...
  const allSelected = selectable.length && selectable.every(row => selectedKeys.value.has(rowKey(row)))
  const next = new Set(selectedKeys.value)
  ...
  <ProjectModal title="导入镜头 Excel" :description="`预检 ${projectName || '当前项目'} 的全部可见 EPnnn Sheet，确认跨 Sheet 行后再以单事务正式提交。`" :busy="isBusy" wide @close="emit('close')">
    <div class="import-flow">
      ...
      <section v-if="commitResult" class="import-success" role="status">
        ...
        <dl><div><dt>提交行</dt><dd>{{ commitResult.committedRows }}</dd></div><div><dt>新建集</dt><dd>{{ commitResult.createdEpisodes }}</dd></div><div><dt>新建场次</dt><dd>{{ commitResult.createdScenes }}</dd></div><div><dt>新建镜头</dt><dd>{{ commitResult.createdShots }}</dd></div><div><dt>新建任务</dt><dd>{{ commitResult.createdTasks }}</dd></div><div><dt>待匹配场景</dt><dd>{{ commitResult.createdAssetRequirements }}</dd></div></dl>
      ...
      <template v-else-if="preview">
        <section class="preview-summary">
          ...
          <div><span>集 / 场次 / 镜头</span><strong>{{ preview.summary.distinctEpisodes }} / {{ preview.summary.distinctScenes }} / {{ preview.summary.distinctShots }}</strong></div>
        ...
        <section v-for="(rows, sheetName) in groupedRows" :key="sheetName" class="sheet-block">
          <header><div><el-icon><Document /></el-icon><strong>{{ sheetName }}</strong><span>{{ rows.length }} 行</span></div><button type="button" @click="toggleSheet(rows)">{{ rows.filter(row => row.canImport).every(row => selectedKeys.has(rowKey(row))) ? '取消本集' : '选择本集有效行' }}</button></header>
          <div class="preview-table-wrap">
            <table><thead><tr><th>选择</th><th>物理行</th><th>场次</th><th>镜头</th><th>时长</th><th>制作人</th><th>制作内容</th><th>场景需求</th><th>预检结果</th></tr></thead>
              <tbody><tr v-for="row in rows" :key="rowKey(row)" :class="{ 'has-errors': row.errors.length, 'has-warnings': row.warnings.length && !row.errors.length }"><td><input type="checkbox" :checked="selectedKeys.has(rowKey(row))" :disabled="!row.canImport" :aria-label="`选择 ${sheetName} 第 ${row.rowNumber} 行`" @change="toggleRow(row)" /></td><td>{{ row.rowNumber }}</td><td>{{ row.normalized?.sceneCode || '—' }} {{ row.normalized?.sceneName || '' }}</td><td>{{ row.normalized?.shotCode || '—' }}</td><td>{{ row.normalized ? `${row.normalized.durationMs} ms` : '—' }}</td><td>{{ row.normalized?.assigneeUserName || '未分配' }}</td><td class="description-cell">{{ row.normalized?.description || '—' }}</td><td>{{ row.normalized?.assetRequirements?.map(item => item.rawName).join('、') || '—' }}</td><td><ul class="issue-list"><li v-if="row.canImport && !row.warnings.length">可导入</li><li v-for="issue in row.warnings" :key="`w-${issue.errorKey}-${issue.fieldName}`" data-tone="warning">{{ issueText(issue) }}</li><li v-for="issue in row.errors" :key="`e-${issue.errorKey}-${issue.fieldName}`" data-tone="danger">{{ issueText(issue) }}</li></ul></td></tr></tbody>
            </table>
          </div>
        </section>

        <footer><div><strong>正式提交将全量回滚任一失败行</strong><p>选中行以 Sheet 名和物理行号共同标识，重复点击会复用同一幂等键。</p></div><el-button :disabled="isBusy" @click="emit('close')">取消</el-button><el-button type="primary" :loading="committing" :disabled="!selectedRows.length" @click="commitImport">正式导入 {{ selectedRows.length }} 行</el-button></footer>
      </template>
    </div>
  </ProjectModal>
</template>
