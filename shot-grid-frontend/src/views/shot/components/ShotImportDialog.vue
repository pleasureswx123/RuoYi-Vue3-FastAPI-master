  ...
  <ProjectModal title="导入镜头 Excel" :description="`预检 ${projectName || '当前项目'} 的全部可见 EPnnn Sheet，确认跨 Sheet 行后再以单事务正式提交。`" :busy="isBusy" wide @close="emit('close')">
    <div class="import-flow">
      ...
      <template v-else-if="preview">
        <section class="preview-summary">
          ...
          <div><span>错误行</span><strong data-tone="danger">{{ preview.summary.errorRows }}<small v-if="overriddenRowCount"> · 已改选 {{ overriddenRowCount }}</small></strong></div>
    ...
    <el-dialog v-model="showBatchAssign" class="import-batch-assign-dialog" title="批量分配制作人" width="480px" append-to-body :z-index="3100" destroy-on-close @closed="batchAssigneeUserId = ''">
      <div class="batch-assign-content">
        ...
        <label class="batch-assign-field"><span>分配给</span><select v-model="batchAssigneeUserId" class="assignee-select batch-assignee-select" aria-label="批量分配镜头制作人"><option disabled value="">请选择制作人</option><option value="__unassigned__">未分配</option><option v-for="member in assignableMembers" :key="member.userId" :value="String(member.userId)">{{ assigneeLabel(member.userId) }}</option></select><small>可选择“未分配”清空当前勾选行的制作人。</small></label>
      </div>
