  ...
  <ProjectModal title="编辑项目" description="项目代号和 NAS 绑定不可在普通编辑中修改。" :busy="busy" @close="emit('close')">
    <form class="edit-form" @submit.prevent="submit">
      ...
      <div class="edit-form__grid">
        <label>
          <span>当前阶段 *</span>
          <select v-model="form.currentPhase">
            <option value="planning">策划</option>
            <option value="asset_production">资产制作</option>
            <option value="shot_production">镜头制作</option>
        ...
        <label>
          ...
          <select v-model="form.aspectRatio">
            <option v-for="ratio in ['16:9', '21:9', '2.39:1', '9:16', '1:1']" :key="ratio">{{ ratio }}</option>
          </select>
        </label>
