...
<template>
  <ProjectModal :title="isReassign ? '改派资产任务' : '分配资产任务'" :description="`${asset.assetName} · ${item.productionItem || '未命名制作分项'}；一个制作分项始终只维护一个资产图片任务。`" :busy="saving" @close="emit('close')">
    <form class="assign-form" @submit.prevent="submit">
      ...
      <label><span>唯一主制作人</span><select v-model="form.assigneeUserId" :disabled="saving"><option value="">请选择</option><option v-for="member in members" :key="member.userId" :value="String(member.userId)">{{ memberLabel(member) }}</option></select></label>
      <label><span>任务要求</span><textarea v-model="form.taskDescription" rows="4" :disabled="saving" placeholder="说明图片交付要求" /></label>
      <div class="assign-form__grid"><label><span>优先级</span><select v-model="form.priority" :disabled="saving"><option value="low">低</option><option value="normal">普通</option><option value="high">高</option><option value="urgent">紧急</option></select></label><label><span>截止日期</span><input v-model="form.dueDate" type="date" :disabled="saving" /></label></div>
