...
镜头导入、新建和编辑仍允许制作信息留空；分配或改派任务前，制作内容描述、景别、机位、镜头运动、焦段五项必须全部为非空白文本。镜头列表、详情及任务可操作项在缺项时不返回 `task.assign`；批量选择包含缺项镜头时不展示批量分配按钮。单条与批量分配接口均在镜头锁内复核，缺项返回 `422 / SG_SHOT_PRODUCTION_FIELDS_REQUIRED` 并列出待补字段，批量事务整体回滚。该规则不增加时长、台词、音效等必填条件。
