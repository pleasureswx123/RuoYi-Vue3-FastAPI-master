...
> **2026-08-10 实现状态校正：**本文第 10 节中的路由默认是目标设计，不等于已有 Controller。当前可调用边界以《[第一批后端联调清单](./第一批后端联调清单.md)》的“已实现”表为准；未出现在该表中的第 10 节接口均为 **计划接口**，前端不得预先假设可用。该清单是 Controller/Service/DAO/VO/DO 和后端契约测试的静态核对结果，不代表已联调或 E2E 通过。
...
返回《项目需求规格与业务规则》第 13 节冻结的全部统计。统计仅包含当前项目 `del_flag='0'` 且未归档的集、场次、镜头、资产和制作分项，由后端统一聚合；镜头的所属集或场次归档后不再计入，制作分项的所属资产归档后也不再计入。`overallProgress` 分母为 0 时返回 `0.0`。`completedAssets` 表示所有活动制作分项均已完成且至少存在一个制作分项的资产数；待审核、修改中资产表示至少一个制作分项处于对应状态。列表分页、镜头筛选以及表格、卡片、故事板视图参数不进入概览聚合查询，不能改变项目总统计值。
