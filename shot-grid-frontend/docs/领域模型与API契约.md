...
- 子行展示自身 `productionItem/description/thumbnail/task/assetStatus/latestVersion`；制作人优先使用平台 `userName`，无任务为未分配。操作列按分项 `allowedActions` 与平台权限交集提供开始任务、分配/改派任务、编辑分项、删除分项和分项详情。写操作先重读原资产详情并冻结目标及锁版本，复用原表单和确认流程；开工仍逐分项提交三份锁版本。详情入口打开原资产抽屉并定位对应分项。
- 镜头、父资产及制作分项的操作列统一使用 `ElButton size="small" round`，所有动作同时显示图标与中文文字，并保留 `aria-label`；不显示悬停提示，必要的禁用原因通过 `aria-description` 提供给辅助技术。开工使用主色实底；分配/改派使用蓝色 `plain`；详情/编辑使用中性 `plain`；补齐制作分项使用警告色 `plain dashed`；删除使用危险色 `plain`。自定义颜色复用已有主题变量，`dark` 绑定 Pinia 主题状态，并校验明暗主题下普通、悬停、按下、焦点及禁用样式。资产操作列加宽至 480px，镜头操作列加宽至 420px，保留换行及横向滚动，完整呈现可用操作。该展示调整不改变任何服务端动作、任务状态或接口契约。
- 制作人以主文字显示，无负责人时弱化；状态使用统一的七色主题变量和 `ElTag size="small" effect="light" round`，父资产计数与子分项、镜头同状态同色，目录失败另外显示红色异常标签。明亮模式为淡底深字，暗黑模式为深底亮字；状态名称和数量始终保留。资产状态列宽度为 200px，优先同排，空间不足时换行。
...
账号或导航初始化失败进入 `/service-unavailable?retry=...` 后，“重新加载”须使用既有站内地址校验和 Vue Router 返回原目标，再由路由守卫重试初始化；不能只刷新公开的异常页。保留目标的项目等查询参数，缺失、站外或异常页自身目标回退 `/`；重试期间禁止重复点击且不主动退出登录，服务持续失败仍保留恢复入口，401/403 继续按原权限规则处理。
...
同一接口支持表格、卡片和故事板。前端不得为三种视图建立三套数据源。`orderByColumn=sortOrder` 时，服务端先按集编号 `episodeNo` 升序排列，再按场次编号 `sceneNo` 升序排列，最后按场内镜头 `(sortOrder, shotNo, shotId)` 排列，禁止仅按镜头排序键跨集或跨场交错。
...
1. 校验项目、集、场次和资产归属。
2. `shotNo` 必填且为 1..214748364 的整数（排序键为编号乘 10）。项目锁内校验同场次全部未删除记录不重号，冲突返回 409 / `SG_SHOT_NO_CONFLICT`；允许任意起号、跳号和乱序，不接受 `sequencePosition/sortOrder`，不重排其他镜头。
...
拖拽只在主动进入“排序模式”且表格已筛选到具体集和具体场次、没有关键字/状态/制作人附加筛选、使用升序排序且已加载完整场次时启用。前端按每页最多 100 条读取全部结果，单场超过 2000 条或仍在加载时禁用拖拽；完整加载后隐藏分页，提交的 `sequencePosition` 必须是整场位置。成功或失败后都重新读取完整场次。不再向用户暴露与拖拽分离的“按当前顺序重新编号”常规动作；下列接口仅保留为历史客户端兼容和人工修复通道：
...
| C | 时长(s) | `durationMs` | 可空；留空为 0，填写秒值精确换算为整数毫秒 |
| D | 镜头缩略图 | — | 只读列，导入忽略 |
| E | 制作内容描述 | `description` | 可空；留空为空字符串，后续编辑补充 |
| F | 景别 | `shotSize` | 文本 |
...
- 解析结果的 `sortOrder` 按 Sheet 内有效数据行顺序生成；正式写入仍使用兼容排序键 `shotNo * 10`，列表保持现有按编号排序。输入行无需按编号递增，不把 Excel 行号保存为业务主键。
...
- 2026-09-08 起，导入保留 Excel 镜头编号，不要求从 `0001` 开始或连续，允许跳号、乱序和补号；提交只复核同场次重号冲突，不重新编号既有或新镜头。正整数格式、权限、项目锁、事务与幂等门禁保持不变。现有拖拽和删除的重排机制尚未解耦，不能把本次导入放宽视为这些操作也已支持自由编号。
...
- 目标尚无任务时创建 `not_started` 任务，此时 `taskLockVersion` 必须为空；已有任务只有仍为 `not_started` 时可普通改派，必须携带当前 `taskLockVersion`，更新原任务并记录审计，不创建第二个任务。
- 已有任务存在任何非 `committed` 版本提交时禁止改派，`failed` 也属于未解决提交；必须重试原提交或使用后续明确治理动作。
- `preparing/in_progress/pending_review/revision/completed` 均视为已开工，禁止普通改派，包括管理人员和全项目范围管理员；写入在锁内校验并返回 HTTP 409 / `SG_INVALID_STATE_TRANSITION`。不重置任务、不改变历史负责人或版本；任务交接与重新开启属于后续独立能力，不在本次提供。
- 镜头、制作分项和任务列表/详情的 `allowedActions` 必须与写入门禁一致：无任务时可返回首次分配；已有任务仅 `not_started` 且不存在非 `committed` 提交时可返回 `task.assign`。所有入口仍保留平台分配权限、项目管理范围、项目可写、NAS 就绪和资源生命周期门禁。前端按唯一任务是否存在区分“分配任务”和“改派任务”，缺少动作投影不得放行。
- 父资产仅在全部活动制作分项均可分配时返回 `task.assign`；镜头和制作分项批量分配必须复用相同的锁内校验，任一已开工目标导致整批回滚，不能自动跳过并部分成功。弹窗打开后开工或改派导致的过期锁版本仍按乐观锁冲突处理，即使提交最新锁版本也不能普通改派已开工任务。
...
制作人工作台和任务详情将时间单独标为正常（剩余大于 24 小时）、临近结束（0 至 24 小时，含 24 小时）、已延期（到达结束时间）；已完成停止催办，无日期显示未设置。提醒每 30 秒及页面恢复可见时本地重算，不新增调度器、推送或业务状态。旧 `dueDate` 按当地当天 23:59:59 兼容提醒。
...
- 制作分项 `allowedActions`：可选 `assetItem.edit`、`assetItem.archive`、`assetItem.delete`、`task.assign`；已有正式版本后不返回普通编辑，活动任务阻止归档，任务已开工（`preparing` 及后续状态）或存在任何非 `committed` 版本提交时不返回分配/改派。
...
| 创建镜头 | 未分配镜头、用户填写且场内唯一的 `shot_no`、镜头资产关系和操作审计；`storage_dir_name` 为空，不创建目录 Outbox 或任务 |
| 导入镜头 | 导入批次、集、场次、未分配且保留 Excel 原编号的镜头（可跳号、乱序）、已匹配资产关系、待匹配资产需求和操作审计；镜头目录 Outbox 与任务创建数均固定为 0 |
...
| `SG_SHOT_SEQUENCE_NOT_CONTIGUOUS` | 409 | 历史迁移的场内连续编号预检失败；Excel 导入自 2026-09-08 起不再返回此错误 |
...
- 版本：`20260831_26 → 20260908_27`，仅 PostgreSQL；新库 SQL 已同步 head。
- 范围：`sg_shot.shot_size/camera_position/camera_movement/focal_length` 扩为 `VARCHAR(500)`，`remark` 扩为 `VARCHAR(2000)`。不回填或改写现有文本，不影响资产及平台表。
- 已有库：先备份，在后端目录执行 `.\.venv\Scripts\ruoyi.exe db upgrade --env dev --revision 20260908_27 --yes`。扩容使用同一事务；表锁获取失败时回滚后重试。
- 恢复：降级先取得排他表锁并检查所有记录。只有全部符合旧上限才能收缩；发现长文本则报 `SG_SHOT_TEXT_DOWNGRADE_BLOCKED`，须保留备份并经业务确认处理长文本后重试，不自动截断。


### 镜头任务分配前置条件（2026-09-08）

镜头导入、新建和编辑仍允许制作信息留空；分配或改派任务前，制作内容描述、景别、机位、镜头运动四项必须全部为非空白文本；焦段允许为空，不影响分配或改派任务。镜头列表、详情及任务可操作项在缺项时不返回 `task.assign`；批量选择包含缺项镜头时不展示批量分配按钮。单条与批量分配接口均在镜头锁内复核，缺项返回 `422 / SG_SHOT_PRODUCTION_FIELDS_REQUIRED` 并列出待补字段，批量事务整体回滚。该规则不增加时长、台词、音效等必填条件。
...
### 镜头分页与显式排序模式（2026-09-09）

镜头列表默认保留分页，选择具体集和场次不再自动加载整场。具备编辑权限、选定集和场次且无附加筛选时，可主动进入“排序模式”，按原有 2000 镜上限加载整场；只有加载完整后才隐藏分页。镜头号连续性、冻结状态和服务端重排校验保持有效。退出排序、提交筛选、重置、切换项目或视图时恢复普通分页并回到第一页；整场加载失败保留错误提示和退出入口。


本期范围调整（2026-09-09）：暂不开放手动拖拽排序，隐藏排序模式入口及全部排序引导提示；保留实现供后续启用，当前统一使用普通分页。
