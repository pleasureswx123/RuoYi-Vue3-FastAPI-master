...
当前已实现版本提交初始化、受保护媒体校验、NAS 原子发布、正式版本与自动审核单入库、状态查询和有限重试；审核动作闭环仍未实现。Excel 模板下载端点也未在本批次生成；预检按仓库内已冻结样表执行。未完成能力必须在后续批次通过 Service 事务和业务测试实现，不能直接使用通用 CRUD 绕过状态机。
