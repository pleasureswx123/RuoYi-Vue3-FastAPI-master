...
- 2026-08-31 已确认在 `20260828_24` 之后扩展统一项目排期：`sg_task.expected_start_time/expected_end_time` 继续作为当前计划，首次完整排期同事务冻结不可变基线，后续改期只更新当前计划；已有完整计划在新迁移中只复制为基线，不伪造历史操作人、原因或时间。无计划任务保持为空，未委派对象不得创建虚拟任务。
...
- 正式契约见 `../shot-grid-frontend/docs/superpowers/specs/2026-08-31-shot-grid-scheduling-timeline-gantt-design.md`。设计确认不等于迁移、接口、查询性能或 E2E 已实现。
