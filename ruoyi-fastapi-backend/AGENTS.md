...
> 2026-08-21 后以 `20260821_17` 为当前 PostgreSQL head：`shot_id` 是稳定内部主键且与 NAS 无关，`shot_no/Sxxx` 只表示从 1 起连续的场内镜序。新建/导入镜头的 `storage_dir_name` 为空，当前制作人开始镜头任务时才冻结 `{sceneNo:03d}_S{shotNo:03d}` 并创建目录 Outbox，任务先进入 `preparing`，Worker 成功后才进入 `in_progress`。拖拽只允许被移动区间全部镜头均未开始制作；`not_started` 可以排序，已开始状态或版本/文件存在时整体禁止。删除镜头必须锁定整场，从最早删除位置到场尾重查同一门禁，并在同事务内连续化剩余镜头；受影响区间已有冻结目录时以 `SG_SHOT_DELETE_DIRECTORY_EXISTS` 拒绝，禁止隐式迁移。17 增加仅审核方可见的问题草稿；只有 `reject` 审核事务才把当前审核单草稿发布为不可变正式问题，`approve` 有草稿时拒绝，`defer` 保留草稿。部署必须先执行 `upgrade 20260821_17` 再启动新代码；旧库若触发 `SG_SHOT_SEQUENCE_NOT_CONTIGUOUS`，必须先在备份和克隆库上通过受控场内重排与 NAS 迁移治理，不能强行 stamp。
...
- 当前 head `20260821_17` 是 PostgreSQL-only 增量链：`20260821_15` 允许 `sg_shot.storage_dir_name` 为空，将镜头目录冻结延迟到任务开始，并增加 `preparing` 状态；`20260821_16` 在任何排序键规范化前校验每场活动镜头按稳定顺序已形成 `S001..Snnn`，冲突以 `SG_SHOT_SEQUENCE_NOT_CONTIGUOUS` 整体失败，通过后只把 `sort_order` 规范为 `shot_no * 10`，不猜测镜头号或 NAS 目录；`20260821_17` 增加 `sg_review_issue_draft`，迁移仅把活动自动审核单中尚未参与处理或确认的 open 当前版问题转为私有草稿。`20260820_14` 将镜头号唯一边界从集调整为场次 `(scene_id, shot_no)`，并增加兼容重编号与 NAS 两阶段迁移 Outbox；`20260820_13` 仅调整 `sg_shot.sort_order` 的数据字典语义。更早 revision 的角色、媒体、审核和归档语义保持不变。
- PostgreSQL 部署必须先执行 `upgrade 20260821_17`，再启动包含场内连续编号、延迟目录冻结、审核草稿发布和平台角色保护链的新代码。固定角色配置完成后，由同时具有 `shotgrid:project:all` 和 `system:user:edit` 的管理员调用 `POST /shot-grid/platform-role-bindings/reconcile` 对账存量成员；对账失败必须整体回滚。管理端不得改名或删除两个固定角色键，不得从通用用户角色入口移除活动项目成员所需或 Shot Grid 拥有来源标记的关系。
...
- 平台管理端通过 `GET/POST/PUT /shot-grid/admin/storage-roots` 和 `POST /shot-grid/admin/storage-roots/{storageRootId}/probe` 维护根目录白名单。探测必须由后端服务账号在事务外对规范化 UNC 根目录执行随机临时文件的独占创建、回读和删除，再以短事务持久化健康状态与同事务操作日志；不得把浏览器所在账号能打开共享目录当成后端可写证据。新增、路径变更或停用后重新启用时状态重置为 `unknown`，只有 `enabled + healthy` 才能进入创建项目选项。
...
- `GET /shot-grid/imports/shots/template` 要求 `shotgrid:shot:import`，直接返回鉴权后的 XLSX 二进制、文件名为 `镜头导入模板-shot-v2.xlsx` 的 `Content-Disposition` 和 `X-Shot-Grid-Template-Version: shot-v2`，不套 JSON envelope。应用层传输加密只精确放行该 GET，不得把同前缀预检或提交 JSON 路由降级为明文。部署资源缺失或摘要不符稳定返回 HTTP 503 / `SG_IMPORT_TEMPLATE_UNAVAILABLE`。
- 镜头打包模板 `module_shot_grid/resources/templates/shot-v2.xlsx` 的冻结 SHA-256 为 `B6F24078CA56295E9E6CCE50BB3455AF198DFFFE5C08F8D85605A68C09439ECE`，主数据区为 A:O 15 列且不含制作人。资产打包模板 `module_shot_grid/resources/templates/asset-v2.xlsx` 的冻结 SHA-256 为 `B551AC1D1D5EDC20A025B0ED90157412E1365006108816F08CB2C59AE4301696`，主数据区为 A:F 且不含制作人。旧 v1 文件只保留为历史资源，不再由模板服务下载。测试必须守住版本、摘要、表头边界以及不存在驱动器路径、`file:` URI、UNC、个人/组织或应用元数据泄露。
...
- 2026-08-11 本批最新后端静态验证记录：`python -m ruff check module_shot_grid config middlewares tests/module_shot_grid` 通过，`python -m ruff format module_shot_grid config middlewares tests/module_shot_grid --check` 报告 161 files already formatted；版本预检 3 个定向测试文件为 43 passed；完整 `tests/module_shot_grid` 为 499 passed、2 skipped，两个跳过项均因当前 Windows 环境不允许创建目录符号链接。静态结果本身不能替代任务/版本浏览器子集的独立证据。
...
- 资产创建时同时冻结 `asset_type/asset_name/asset_name_key/storage_dir_name/storage_path_key`；普通编辑只允许描述、排序和备注，重命名、改类型或目录迁移必须使用后续受控动作。制作分项仅在未分配或唯一任务仍为 `not_started` 且尚无版本时可补充；制作人开始任务后主数据立即冻结，不等待首个版本提交。资产图片版本提交前必须补齐制作分项。资产及制作分项响应中的 `allowedActions` 是后端根据平台权限、项目角色、项目状态、存储状态、资源生命周期、任务/版本及未提交版本发布状态计算的唯一动作依据，前端不得自行合成。制作分项缩略图只取该分项当前最新版本的 `thumbnail` 文件，不回退旧版本；父资产代表图按活动分项 `(sort_order, asset_item_id)` 顺序选择第一张可用的当前最新版本缩略图。旧正式版本事务当前按 `project → task/submission → version → auto_single review list → note` 顺序加锁；新审核闭环必须扩展为问题处理说明和确认记录的稳定锁序，避免项目元数据、改派、提交和审核并发穿透。
- 任务列表提供项目范围与跨项目 `GET /shot-grid/tasks/mine`；独立业务前端的 `/workbench` 使用该跨项目真实数据源，`/tasks/:taskId` 使用任务详情、开始和编辑真实接口。镜头任务详情额外返回详情专用 `shotProduction`，投影当前镜头的时长、制作内容、景别、机位、镜头运动、焦段、台词/对白、音效、色调参考和备注；任务列表仍只返回目标摘要，不把完整制作信息扩入列表响应。动作必须同时满足平台权限与后端 `allowedActions`。任务编辑继续按项目管理权限复核；开始任务则必须在行锁内确认当前用户就是任务当前 `assignee_user_id`、对应成员仍为 `active + creator` 且平台账号有效，`director`、管理员、超级管理员和 `shotgrid:project:all` 均不得代开始。首次分配的 `taskLockVersion` 必须为空，已有任务改派时必须携带当前 `taskLockVersion`，开始任务必须携带 `lockVersion`。任务存在任何非 `committed` 提交（包括 `failed`）时禁止改派，只允许当前活动负责人重试原提交或走后续明确治理动作。
- 版本提交当前实现固定为“本地文件校验 → `POST /shot-grid/tasks/{taskId}/version-submissions/preflight` → `POST /common/files/upload` 平台私有上传 → `POST /shot-grid/tasks/{taskId}/version-submissions` 创建提交并返回 HTTP 202”，旧预检请求体为 `fileName/fileSize/changelog/aiParams`。新正式契约要求 preflight/create 同时携带 `issueResponses`：首版为空，修订版精确覆盖锁内重查的全部 open 问题，并纳入幂等命令哈希；问题集合变化必须冲突，不能静默接受过期说明。preflight、create 与失败提交 retry 都必须校验当前用户就是任务当前委派的活动 `creator` 本人，不接受 `director`、管理员或全项目范围代提交。其余 NAS 无覆盖发布、文件引用切换、`committed` 反查版本和唯一临时文件规则保持不变。
