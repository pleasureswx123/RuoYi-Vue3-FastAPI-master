...
- 项目创建/编辑/归档、成员变更、集/场次/镜头/资产普通管理、任务分配/改派/开始、Excel 正式提交、版本正式提交、审核意见/回复/解决、审核动作和目录人工重试必须由 Service 在同一数据库事务写领域数据、必要的 Outbox/文件引用与 `SysOperLog`；不得使用会异步入 Redis 的平台 `@Log` 冒充同事务审计。项目、集、场次、镜头、资产和制作分项已实现业务归档而非物理删除；独立业务前端的项目列表、创建、详情/概览、编辑/归档、成员维护和存储诊断已接入真实接口，并已通过生产 Nginx 形态下的隔离项目管理浏览器旅程。资产需求人工处理、`manual_batch` 审核单、媒体派生及镜头/资产/任务/版本/审核等其余业务页面仍待后续实现；该项目管理子集验证不是完整系统 E2E 或生产就绪结论。
...
- 2026-08-11 本批验证记录：Ruff check/format 覆盖 157 个 Python 文件并通过；定向测试 26 passed；完整 `tests/module_shot_grid` 为 441 passed、2 skipped，跳过项是当前 Windows 账号没有创建符号链接权限；四条项目选项/候选路由及权限已由 OpenAPI/路由测试确认。隔离 PostgreSQL 由当前初始化基线创建并处于 `20260811_06`，Redis 使用隔离 DB 15，真实平台管理员账号经生产 Nginx 完成项目管理子集浏览器旅程。该旅程中的存储根是逻辑 `healthy` 夹具、目录 Worker 关闭，项目存储仍为 `initializing` 且 `initialize_project` 仍为 `pending`，因此没有验证真实 UNC/SMB、Windows 服务账号、共享 ACL 或物理目录创建。
