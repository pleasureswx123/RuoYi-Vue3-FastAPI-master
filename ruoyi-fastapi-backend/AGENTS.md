...
> 2026-08-25 后以 `20260825_19` 为当前 PostgreSQL head：`shot_id` 是稳定内部主键且与 NAS 无关，`shot_no/Sxxx` 只表示从 1 起连续的场内镜序。新建/导入镜头和资产均不创建对象目录 Outbox；资产仍在创建时冻结稳定目录身份。当前制作人开始镜头或资产任务时才创建目录 Outbox，任务先进入 `preparing`，Worker 成功后才进入 `in_progress`。镜头开始时同时冻结 `{sceneNo:03d}_S{shotNo:03d}`；同一父资产的多个制作分项共享资产目录并通过父资产行锁复用幂等操作。拖拽只允许被移动区间全部镜头均未开始制作；`not_started` 可以排序，已开始状态或版本/文件存在时整体禁止。删除镜头必须锁定整场，从最早删除位置到场尾重查同一门禁，并在同事务内连续化剩余镜头；受影响区间已有冻结目录时以 `SG_SHOT_DELETE_DIRECTORY_EXISTS` 拒绝，禁止隐式迁移。部署必须先执行 `upgrade 20260825_19` 再启动新代码；旧库若触发 `SG_SHOT_SEQUENCE_NOT_CONTIGUOUS`，必须先在备份和克隆库上通过受控场内重排与 NAS 迁移治理，不能强行 stamp。
...
- 当前 head `20260825_19` 是 PostgreSQL-only 增量链：`20260821_15` 允许 `sg_shot.storage_dir_name` 为空，将镜头目录冻结延迟到任务开始，并增加 `preparing` 状态；`20260821_16` 在任何排序键规范化前校验每场活动镜头按稳定顺序已形成 `S001..Snnn`；`20260821_17` 增加审核问题私有草稿；`20260825_18` 增加项目永久删除审计与清理队列；`20260825_19` 删除“preparing 仅限镜头”的旧约束，将资产目录物理创建延迟到资产任务开始。`20260820_14` 的镜头重编号与更早 revision 的角色、媒体、审核和归档语义保持不变。
- PostgreSQL 部署必须先执行 `upgrade 20260825_19`，再启动包含场内连续编号、镜头/资产延迟目录、审核草稿发布、项目永久删除和平台角色保护链的新代码。固定角色配置完成后，由同时具有 `shotgrid:project:all` 和 `system:user:edit` 的管理员调用 `POST /shot-grid/platform-role-bindings/reconcile` 对账存量成员；对账失败必须整体回滚。管理端不得改名或删除两个固定角色键，不得从通用用户角色入口移除活动项目成员所需或 Shot Grid 拥有来源标记的关系。
