...
- 项目创建/编辑/归档、成员变更、集/场次/镜头/资产普通管理、任务分配/改派/开始、Excel 正式提交、版本正式提交、审核意见/回复/解决、审核动作和目录人工重试必须由 Service 在同一数据库事务写领域数据、必要的 Outbox/文件引用与 `SysOperLog`；不得使用会异步入 Redis 的平台 `@Log` 冒充同事务审计。项目、集、场次、镜头、资产和制作分项已实现业务归档而非物理删除；独立业务前端的项目、镜头、资产、跨项目“我的任务”工作台、任务详情/开始/编辑、版本提交恢复、版本历史/详情和鉴权下载已接入真实接口，不回退 Mock。项目管理、镜头管理/镜头 Excel 导入、资产管理/资产 Excel 导入、任务工作台/版本上传四个子集均已完成隔离 PostgreSQL、Redis、真实平台账号、生产 Nginx 与 Chrome 浏览器旅程。任务/版本旅程只使用显式 `allow_local_root=True` 的 TEMP 适配器验证发布算法和前后端编排，不是正式 UNC/SMB/NAS 服务账号验收。资产需求人工处理、`manual_batch` 审核单、媒体派生、审核前端和文件与 NAS 一级页面仍待后续实现；任何子集旅程均不等于完整系统 E2E 或生产就绪。
...
- 版本提交固定为“本地文件校验 → `POST /shot-grid/tasks/{taskId}/version-submissions/preflight` → `POST /common/files/upload` 平台私有上传 → `POST /shot-grid/tasks/{taskId}/version-submissions` 创建提交并返回 HTTP 202”。预检请求体固定为 `fileName/fileSize/changelog/aiParams`，只读、无数据库或文件副作用，只验证目标相对路径可生成和目录快照字段完整；它不访问 NAS，也不检查实际目标文件。正式创建在锁定项目、任务和源文件后全量复检权限、状态、归属、文件、目标相对路径生成、目录快照一致性及未解决提交。创建提交后先建立临时 `businessType=shotgrid_version_submission` 文件引用，再由默认关闭的版本发布 Worker 执行 NAS 摘要校验和无覆盖原子发布，最后在短事务中创建正式版本、主文件引用和 `auto_single` 审核单。实际目标文件已存在的摘要冲突只在 Worker 发布阶段处理；`committed` 的 `versionId` 通过 `sg_version.submission_id` 反查，`sg_version_submission` 不重复保存 `version_id`。每次领取使用含 attempt 和随机值的唯一临时文件名；目标文件已存在时仅摘要和大小完全一致才按幂等成功处理，禁止覆盖。
