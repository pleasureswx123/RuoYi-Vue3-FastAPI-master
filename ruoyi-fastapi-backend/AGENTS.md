...
- Shot Grid 领域模块位于 `module_shot_grid/`，包含 22 张 `sg_` 表 DO、项目访问依赖、范围导航、项目创建/存储状态/成员/范围查询、镜头制作人选项、镜头导入模板下载、镜头和资产 Excel 预检与正式提交、独立任务管理、版本提交/NAS 发布、`auto_single` 自动审核闭环，以及 NAS 目录 Outbox Worker、目录操作查询和人工重试接口。
...
- 项目创建/编辑/归档、成员变更、集/场次/镜头/资产普通管理、任务分配/改派/开始、Excel 正式提交、版本正式提交、审核意见/回复/解决、审核动作和目录人工重试必须由 Service 在同一数据库事务写领域数据、必要的 Outbox/文件引用与 `SysOperLog`；不得使用会异步入 Redis 的平台 `@Log` 冒充同事务审计。项目、集、场次、镜头、资产和制作分项已实现业务归档而非物理删除；独立业务前端的项目列表、创建、详情/概览、编辑/归档、成员维护、存储诊断，以及镜头列表三视图、详情、创建/编辑/归档、任务分配和镜头 Excel 预检/提交已接入真实接口，不回退 Mock。项目管理子集和本批镜头管理/镜头 Excel 导入子集均已完成隔离 PostgreSQL、Redis、真实平台账号、生产 Nginx 与 Chrome 浏览器旅程。资产需求人工处理、`manual_batch` 审核单、媒体派生及资产/任务/版本/审核等其余业务页面仍待后续实现；两个子集旅程均不等于完整系统 E2E 或生产就绪。
...
- 集、场次、镜头的创建、修改、归档在锁定并读取项目后拒绝 `completed`、`archived`；镜头 Excel preview 普通读取项目状态并拒绝，commit 再以 `FOR UPDATE` 锁定项目重检，统一返回 HTTP 409 / `SG_INVALID_STATE_TRANSITION`。项目详情在 `completed` 时只允许合法的 `project.archive`，在 `archived` 时不返回写动作；镜头 `allowedActions` 同步为空。该门禁目前只完成项目自身、集、场次、镜头和镜头导入这一批明确路径，不能推断资产、成员、任务、版本、审核、文件或目录操作等全域写接口已经统一完成终态治理。
- 项目成员添加、修改和移除在锁定项目行后必须重新解析当前操作者的项目访问范围，并再次要求 `director`，防止操作者在等锁期间被移除或降级后仍完成写入；Controller 前置依赖不能替代该锁内复核。
- 2026-08-11 本批最新验证记录：Ruff check/format 覆盖 163 个 Python 文件并通过；模板、项目选项、终态写门禁及镜头导入等 10 个定向测试文件为 78 passed；完整 `tests/module_shot_grid` 为 465 passed、2 skipped，跳过项是当前 Windows 账号没有创建符号链接权限。模板二进制契约、镜头制作人选项路由和权限已由 OpenAPI/路由测试确认。镜头管理/镜头 Excel 导入子集还在隔离 PostgreSQL、Redis DB 15、真实 FastAPI/平台账号、生产 Nginx 和 Chrome 下完成浏览器旅程：下载模板 11883 bytes 且摘要匹配，preview 为 24/24、0 警告行、0 错误行、2 集/8 场/24 镜头，选中 24 行后 commit HTTP 200，创建 2 集、8 场、24 镜头、24 任务、24 待匹配需求和 26 条目录操作。数据库终态与 UI 一致，Redis 预检键提交后为 0，浏览器控制台 0 error/0 warning，退出后详情深链回登录页。该旅程使用逻辑 `storageStatus=ready` 夹具且目录 Worker 关闭，26 条目录操作仍为 `pending`；它没有验证真实 UNC/SMB、Windows 服务账号、共享 ACL 或物理目录创建，也不是完整系统 E2E。
